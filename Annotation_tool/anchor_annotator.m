...
for i= 1:handles.part_num
    if( handles.(cad(handles.cad_index).pnames{i}).status ==1)
        continue;
    else
...
if num_anch < 2
    ...
    v0 = zeros(7,1);
    % azimuth
    a = viewpoint.azimuth_coarse;  %Azimuth : ���ȣ� Yaw
    v0(1) = a*pi/180;
    margin = 22.5;
    ...
    v0(2) = e*pi/180;
    margin = 22.5;
    eextent = [max(e-margin,-90)*pi/180 min(e+margin,90)*pi/180];        %Elevation: Pitch
    % distance
    dextent = [0, 100];                                                  % Distance : Compute by scale
    ...
    % focal length
    v0(4) = 1;
    fextent = [1 1];                                                      % Focal Length ,set by camera matrix
    % principal point
    ...
    rextent = [-pi, pi];
    % lower bound
    lb = [aextent(1); eextent(1); dextent(1); fextent(1); -pi/2; -pi/2; rextent(1)];
    % upper bound
    ub = [aextent(2); eextent(2); dextent(2); fextent(2); pi/2; pi/2; rextent(2)];
...
function [rotation, lb, ub] = compute_principal_point(a, e, d, x2d, x3d)
...
x(2,:) = x(2,:) ./ x(3,:);
x = x(1:2,:)';
% Project direct to the camera plane
...
imgsize = [760, 368];
offset = (center - imgsize/2)/(M*f);
alpha = atan(offset(1)/d);
phi = atan(offset(2)/d);
rotation = [alpha, phi];
...
alpha = principal_point(1);
phi = principal_point(2);
...
x = (R2d * x)';
% compute error
error = normal_dist(x, x2d);

% re-projection error
function error = normal_dist(x, x2d)
imgsize = [760, 368];
error = 0;
for i = 1:size(x2d, 1)
    point = x2d(i,:) - imgsize/2;
    point(2) = -1 * point(2);
...
x(:,2) = -1 * x(:,2);
x = x + repmat(imgsize/2, size(x,1), 1);
